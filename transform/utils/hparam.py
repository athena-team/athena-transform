...
def _cast_to_type_if_compatible(name, param_type, value):
    ...
    if (issubclass(param_type, numbers.Integral)
            and not isinstance(value, numbers.Integral)):
        raise ValueError(fail_msg)

    # Avoid converting a non-numeric type to a numeric type.
    if (issubclass(param_type, numbers.Number)
        ...
        def remove_callables(x):
            ...
            if isinstance(x, dict):
                return {
                    ...
                    for k, v in six.iteritems(x) if not callable(v)
                }
            elif isinstance(x, list):
                return [remove_callables(i) for i in x if not callable(i)]
            return x

        return json.dumps(remove_callables(self.values()),
                          indent=indent,
