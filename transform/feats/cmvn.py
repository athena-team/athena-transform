...
# ==============================================================================
"""This model does CMVN on features."""
    ...
    def call(self, audio_feature, speed=1.0):
        ...
        if self.global_cmvn:
            audio_feature = (audio_feature -
                             params.global_mean) / params.global_variance

        if params.local_cmvn:
            mean, var = tf.compat.v1.nn.moments(audio_feature, axes=0)
            audio_feature = (audio_feature - mean) / (tf.compat.v1.math.sqrt(var) +
                                                      1e-6)
...
def compute_cmvn(audio_feature, mean=None, variance=None, local_cmvn=False):
    ...
    :param audio_feature: A tensor.
    :param mean: A tensor.
    :param variance: A tensor.
    :param local_cmvn: A tensor.
    :return: A tensor.
    """
    if mean is not None:
